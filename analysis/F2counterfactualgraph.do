...
keep year maritalbr nonmaritalbr propfemmarried1544

gen constantpop = 1000000 // it doesn't matter what this is for calculating the ratios

// generate a nmfratio - this will match T1
gen nmfratio = (((1-propfemmarried1544)*nonmaritalbr*constantpop)/(((1-propfemmarried1544)*nonmaritalbr*constantpop)+((propfemmarried1544)*maritalbr*constantpop))*100)
...
gen propmarried09 = propfemmarried1544 if year == 2009
ereplace propmarried09 = max(propmarried09)


// If everything stayed the same except....
// PROP MARRIED
gen propmarriedconstant_counter = (((1-propfemmarried1544)*nmaritalfert09*constantpop)/(((1-propfemmarried1544)*nmaritalfert09*constantpop)+((propfemmarried1544)*maritalfert09*constantpop))*100)
...
graph export "$nmfratioresults/prints/counterfactual_overall.png", replace 

// EDITED HERE FOR DEMOGRAPHY ARTICLE
// no title
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   (line 	nmfratio year, msymbol(O) lcolor(black)) ///
	   (scatter propmarriedconstant_counter year, msymbol(S) mcolor("166 206 227")) ///
	   ...
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lpattern(dash) lcolor("178 223 138")), ///
			xlabel(,labsize(3)) xlabel(2009(2)2023) plotregion(style(none))	///
			legend( order(1 "As observed" 3 "Percentage married" 5 "Marital birth rate" 7 "Nonmarital birth rate")) ///
					ytitle("{bf:Non-marital Fertility Ratio (NMFR)}") ///
		xtitle("{bf:Year}") ///
				ylabel(,angle(0)) 
graph export "$nmfratioresults/prints/F2counterfactual_overall_notitle_fordemog.pdf", replace 

exit

// no longer doing this by educ group.

** RERUN BY EDUCATION GROUP **
import excel "/Users/kristenburke/Library/CloudStorage/Box-Box/UT/Dissertation/Ch 1.5 - Marriage/analysis/data/raw/nmfrcounter_byeduc.xlsx", firstrow clear
destring year, replace
gen constantpop = 1000000 
...
// Generate counterfactual rates compared to 2009
keep if year >= 2009 // & year < 2020
gen maritalfert09 = maritalfertrate if year == 2009
ereplace maritalfert09 = max(maritalfert09)

gen nmaritalfert09 = nonmaritalfr if year == 2009
ereplace nmaritalfert09 = max(nmaritalfert09)
...
// gen nmfratio
gen nmfratio = (((1-propfemmarried1544)*nonmaritalfr*constantpop)/(((1-propfemmarried1544)*nonmaritalfr*constantpop)+((propfemmarried1544)*maritalfertrate*constantpop))*100)
...
// MARITAL FERTILITY RATE
gen marfertrateconstant_counter = (((1-propmarried09)*nmaritalfert09*constantpop)/(((1-propmarried09)*nmaritalfert09*constantpop)+((propmarried09)*maritalfertrate*constantpop))*100) 


// NONMARITAL FERTILITY RATE
gen nonmarfertrateconstant_counter = (((1-propmarried09)*nonmaritalfr*constantpop)/(((1-propmarried09)*nonmaritalfr*constantpop)+((propmarried09)*maritalfert09*constantpop))*100) 

// Graph
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   (line 	nmfratio year, msymbol(O) lcolor(black)) ///
	   (scatter propmarriedconstant_counter year, msymbol(S) mcolor("166 206 227")) ///
	   (line 	propmarriedconstant_counter year, msymbol(S) lcolor("166 206 227")) ///
	   (scatter marfertrateconstant_counter year, msymbol(D) mcolor("31 120 180")) ///
	   (line 	marfertrateconstant_counter year, msymbol(D) lcolor("31 120 180")) ///
	   (scatter nonmarfertrateconstant_counter year, msymbol(T) mcolor("178 223 138")) ///
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lcolor("178 223 138")), ///
			title("High school or less", size(medium)) ///
			xlabel(,labsize(3)) xlabel(2009(2)2019) plotregion(style(none))	///
			legend( order(1 "As observed" 3 "Proportion married" 5 "Marital fertility rate" 7 "Nonmarital fertility rate"))
graph export "$nmfratioresults/prints/counterfactual_highschool.png", replace 
...
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   ...
	   (scatter nonmarfertrateconstant_counter year, msymbol(T) mcolor("178 223 138")) ///
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lcolor("178 223 138")), ///
			title("Some college", size(medium)) ///
			xlabel(,labsize(3)) xlabel(2009(2)2019) plotregion(style(none))	///
			legend( order(1 "As observed" 3 "Proportion married" 5 "Marital fertility rate" 7 "Nonmarital fertility rate"))
graph export "$nmfratioresults/prints/counterfactual_somecollege.png", replace 
...
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   ...
	   (scatter nonmarfertrateconstant_counter year, msymbol(T) mcolor("178 223 138")) ///
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lcolor("178 223 138")), ///
			title("College degree", size(medium)) ///
			xlabel(,labsize(3)) xlabel(2009(2)2019) plotregion(style(none))	///
			legend( order(1 "As observed" 3 "Proportion married" 5 "Marital fertility rate" 7 "Nonmarital fertility rate"))
graph export "$nmfratioresults/prints/counterfactual_collegedeg.png", replace 
...
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   ...
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lpattern(dash) lcolor("178 223 138")), ///
		...
		xtitle("{bf:Year}") ///
				ylabel(,angle(0)) 
graph export "$nmfratioresults/prints/F2.counterfactual_overall_notitle_fordemog.pdf", replace 
