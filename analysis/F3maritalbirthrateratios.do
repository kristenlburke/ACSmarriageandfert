...
import excel "$nmfratioresults/acsrates/birthrates_marstat.xlsx", firstrow clear
rename *, lower
destring year, replace
...
import excel "$nmfratioresults/acsrates/birthrates_marstat_educ.xlsx", firstrow clear
rename *, lower
destring year, replace
destring maritalstatus, replace
...
forvalues e = 1/3 {
	...
	twoway (line rate year if educ == `e' & maritalstatus == 0,  lcolor(black)) ///
	   ...
	   graph export "$nmfratioresults/prints/marital_nonmarital_rates_educ`e'.png", replace 
}
...
twoway (line rate year if educ == 1 & maritalstatus == 0,  lcolor(black)) /// 
   ...
   graph export "$nmfratioresults/prints/marital_nonmarital_rates_alleducs.png", replace 
...
twoway (line mbrr year if educ == 1, lwidth(medthick) lcolor("102 194 165") lpattern(solid)) ///
	   ...
	   ytitle("{bf:Marital birth rate ratio}") ///
	   xtitle("{bf:Year}") ///
	   ytitle() legend(cols(3))  ///
	   xlabel(,labsize(3)) xlabel(2006(2)2023) plotregion(style(none)) ///
	   ylabel(,angle(0)) 
graph export "$nmfratioresults/prints/F3MBRR by educ_fordemog.pdf", replace 
