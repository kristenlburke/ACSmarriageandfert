...
keep year maritalbr nonmaritalbr propfemmarried1944

gen constantpop = 1000000 // it doesn't matter what this is for calculating the ratios

// generate a nmfratio - this will match T1
gen nmfratio = (((1-propfemmarried1944)*nonmaritalbr*constantpop)/(((1-propfemmarried1944)*nonmaritalbr*constantpop)+((propfemmarried1944)*maritalbr*constantpop))*100)
...
gen propmarried09 = propfemmarried1944 if year == 2009
ereplace propmarried09 = max(propmarried09)


// If everything stayed the same except....
// PROP MARRIED
gen propmarriedconstant_counter = (((1-propfemmarried1944)*nmaritalfert09*constantpop)/(((1-propfemmarried1944)*nmaritalfert09*constantpop)+((propfemmarried1944)*maritalfert09*constantpop))*100)
...
twoway (scatter nmfratio year, msymbol(O) mcolor(black)) ///
	   ...
	   (line 	nonmarfertrateconstant_counter year, msymbol(T) lcolor("178 223 138")), ///
			title("Figure 2. Nonmarital fertility ratio, as observed and standardized to 2009" "save for listed component of change", size(medium)) ///
			xlabel(,labsize(3)) xlabel(2009(2)2022) plotregion(style(none))	///
			legend( order(1 "As observed" 3 "Proportion married" 5 "Marital fertility rate" 7 "Nonmarital fertility rate"))
graph export "$nmfratioresults/prints/counterfactual_overall_1944.png", replace 
