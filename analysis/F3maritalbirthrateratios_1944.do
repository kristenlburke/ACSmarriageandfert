...
forvalues e = 1/3 {
	...
	twoway (line rate year if educ == `e' & maritalstatus == 0,  lcolor(black)) ///
	   ...
	   graph export "$nmfratioresults/prints/marital_nonmarital_rates_educ`e'_1944.png", replace 
}
...
twoway (line rate year if educ == 1 & maritalstatus == 0,  lcolor(black)) /// 
	   ...
	   legend(order(1 "Unmarried, HS" 2 "Married, HS" 3 "Unmarried, some college" 4 "Married, some college" 5 "Unmarried, BA+" 6 "Married, BA+")) 
	   
   graph export "$nmfratioresults/prints/marital_nonmarital_rates_alleducs_1944.png", replace 
...
twoway (line mbrr year if educ == 1, lwidth(medthick) lcolor("102 194 165") lpattern(solid)) ///
	   ...
	   title("Figure 3. Marital birth rate ratio by education, 2003-2019") ///
	   xlabel(,labsize(3)) xlabel(2003(2)2022)
graph export "$nmfratioresults/prints/MBRR by educ_1944.png", replace 
